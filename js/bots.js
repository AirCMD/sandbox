...
class BotEngine {
  ...
  makeComment(rel, mood, postText, speakerId, usedSet = null, authorId = null) {
    ...
    // Ключі реакцій з крафту / data.js
    if (typeof COMMENT_REACTIONS !== "undefined") {
      for (const block of COMMENT_REACTIONS) {
        try {
          if (block.keys && new RegExp(block.keys, "i").test(pt)) {
            pool = [...(block.lines || [])];
            break;
          }
        } catch (_) {}
    ...
    if (!pool.length) {
      const byRel = (typeof COMMENT_BY_REL !== "undefined" ? COMMENT_BY_REL : {})[rel];
      if (byRel && byRel.length) {
        pool = [...byRel];
        if (speaker?.id === "jini" && rel === "close") pool = pool.concat(["О, є деталі? Я вся увага"]);
      } else if (mood === "сумний") {
        ...
        pool = [...(typeof COMMENT_GENERIC !== "undefined" ? COMMENT_GENERIC : [
          "Цікавий погляд", "Має сенс", "Дякую, що {a:написала/написав}"
        ])];
      }
    }
  ...
  getGalleryCommentOptions(item) {
    ...
    if (/зимбач/.test(blob)) add(["Виглядає як побачення.", "Мені здається це вже стосунки між чоловіком та жінкою", "Напевно стосунки"]);
  ...
  craftReply(toId, fromId, userMessage, ctx) {
    ...
    if (/^(привіт|привітик|хай|хел+о|здоров|вітаю|йо)\b/.test(msg) || msg.length < 12 && /привіт|хай/.test(msg)) {
      const greet = (typeof CHAT_CRAFT !== "undefined" && CHAT_CRAFT.greetings?.length)
        ? CHAT_CRAFT.greetings
        : ["Привіт 😊", "О, привіт! Як ти?", "Привітики. Що новенького?"];
      return pick(greet.map(g => g.includes("Привіт") && name ? g.replace("Привіт", "Привіт" + (name ? ", " + name : "")) : g));
    }

    // «Як справи?» — НЕ повторювати те саме питання
    if (/як справи|як ти\b|як життя|що робиш|що робиш\?|як воно|шо робиш/.test(msg)) {
      let byStatus = (typeof CHAT_CRAFT !== "undefined" && CHAT_CRAFT.howAreYou?.length)
        ? CHAT_CRAFT.howAreYou.map(s => s.replace(/\{status\}/g, status || "свої справи"))
        : [
    ...
    if (/сумн|поган|важк|самот|не можу|втоми/.test(msg)) {
      const sup = (typeof CHAT_CRAFT !== "undefined" && CHAT_CRAFT.support?.length)
        ? CHAT_CRAFT.support
        : ["Чую тебе. Якщо треба виговоритись — я тут.", "Ох… тримайся.", "Шкода, що так."];
      return pick(sup);
    }
    ...
    if (/^(так|ні|неа|ага|угу|добре|окей|ок)\b/.test(msg)) {
      const yn = (typeof CHAT_CRAFT !== "undefined" && CHAT_CRAFT.yesNo?.length)
        ? CHAT_CRAFT.yesNo
        : ["Ок.", "Добре.", "Зрозуміла/зрозумів.", "Гаразд."];
      return pick(yn);
    }
